...
public class TestRastererMethods {
    ...
    public void testBinarySearch() {
        ...
        // Testing depth 3
        int x2 = r.binarySearch(r.lonCache, 3, ullon); // 4
        int y2 = r.binarySearch(r.latCache, 3, -1*ullat); // 4
        Assert.assertEquals(x2, 4);
        Assert.assertEquals(y2, 4);

        // Testing out of range, lon
        int x3 = r.binarySearch(r.lonCache, 6, -125); // 0
        int x4 = r.binarySearch(r.lonCache, 6, -120); // 63
        Assert.assertEquals(x3, 0);
        Assert.assertEquals(x4, 63);

        // Testing out of range, lat
        int y3 = r.binarySearch(r.latCache, 6, -40); // 0
        int y4 = r.binarySearch(r.latCache, 6, -30); // 63
